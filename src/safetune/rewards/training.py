"""
Reward Model Training System for SafeTune.
...
# NOTE: ``transformers.Trainer`` / ``TrainingArguments`` transitively import
# torchvision, which can fail on CPU-only / partial installs. These symbols are
# therefore imported lazily inside the functions that need them so that simply
# importing ``safetune.rewards`` never fails over an optional training-only
# dependency. Type annotations referring to them use string ("forward-ref")
# form for the same reason.


def _import_hf_training():
    """Lazily import the heavy HF training symbols used by this module."""
    ...
    return AutoTokenizer, AutoModelForSequenceClassification, TrainingArguments, Trainer

from .base import CompositeReward, RewardFunction
...
class RewardModelLoader:
    ...
    def load_from_huggingface(self, model_name: str, **kwargs) -> "AutoModelForSequenceClassification":
        ...
        try:
            AutoTokenizer, AutoModelForSequenceClassification, _, _ = _import_hf_training()
...
class RewardModelTrainer:
    ...
    def generate_training_data(
        ...
        try:
            AutoTokenizer, _, _, _ = _import_hf_training()
    ...
    def train_reward_model_torch_loop(
        ...
        **kwargs
        ...
        try:
            ...
            # --------------------------------------------------------------
            try:
                _, AutoModelForSequenceClassification, _, _ = _import_hf_training()
                model = AutoModelForSequenceClassification.from_pretrained(
                    self.base_model_name,
                    num_labels=1,
                    torch_dtype=torch.float32
