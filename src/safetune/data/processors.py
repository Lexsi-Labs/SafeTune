...
from safetune.data.schemas import TaskType, TASK_SCHEMAS
...
class SystemPromptInjector:
    ...
    def _apply_chat_template(self, messages):
        ...
        # Only append the assistant generation prompt when the conversation
        # ENDS on a user turn (we're prompting for a NEW response). For an
        # already-complete conversation (last turn = assistant), adding it
        # appends a spurious trailing assistant header and corrupts the text.
        _add_gen = bool(messages) and messages[-1].get("role") == "user"
        # Try with enable_thinking first
        try:
            result = self.tokenizer.apply_chat_template(
                messages,
                tokenize=False,
                add_generation_prompt=_add_gen,
