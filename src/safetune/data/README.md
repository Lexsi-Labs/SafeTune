# Unified Dataset Handler for SafeTune

The **Unified Dataset Handler** (`DataManager`) is the central engine for loading, normalizing, and preparing data for all SafeTune algorithms (SFT, DPO, GRPO, etc.). It eliminates the need for manual data parsing and ensures consistency across training and evaluation.
...
```{python}
from safetune.data.manager import DataManager
...
The manager automatically detects column names based on the task type. It standardizes them into the internal format required by SafeTune trainers.
