...
class PerplexityMetric(Metric):
    ...
    def compute(self, predictions: List[Any], references: List[Any], **kwargs) -> Dict[str, float]:
        ...
        # predictions are per-sample LOSS values (see docstring); perplexity is
        # exp(mean loss). The old code returned the mean loss itself, labelled
        # "perplexity" — wrong (and mean(exp) != exp(mean) regardless).
