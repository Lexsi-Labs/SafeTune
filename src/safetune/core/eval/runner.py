...
try:
    ...
    def SafeTune_info(msg, prefix="[SafeTune]"):
        print(f"{prefix} INFO - {msg}")
    
    def SafeTune_warning(msg, prefix="[SafeTune]"):
        print(f"{prefix} WARNING - {msg}")
    
    def SafeTune_success(msg, prefix="[SafeTune]"):
        print(f"{prefix} ✓ {msg}")
...
def get_metrics_from_names(metric_names: List[str], k_list: Optional[List[int]] = None) -> List[Any]:
    ...
    metrics_map = {
        ...
        "over_refusal": OverRefusalMetric,
        "jailbreak_success": JailbreakSuccessMetric,
        "capability_regression": CapabilityRegressionMetric,
    }
...
def run_eval(config: EvalConfig, dataset_dict: Optional[DatasetDict] = None) -> Dict[str, Any]:
    ...
    print_section_banner("SafeTune EVALUATION", color=Fore.CYAN if COLORED_LOGGING_AVAILABLE else "", use_ascii=True)
    SafeTune_info(f"Model: {config.model_path}")
    SafeTune_info(f"Eval Task (Metrics): {config.task_type}")
    ...
    if config.use_vllm:
        ...
        if is_peft:
            ...
            SafeTune_info(f"Loading LoRA Adapter: {config.model_path} on Base: {base_path}")
            ...
            try:
                model = model.merge_and_unload()
                SafeTune_success("Successfully merged LoRA adapter into base model.")
    ...
    SafeTune_info(f"Loading Dataset via DataManager...")
    ...
    if config.data_task_type:
        ...
        task_mapping = {
            ...
            # Safety eval: safety suite metrics on prompt/response data
            "safety": "sft",
    ...
    try:
        ...
        if missing_cols:
            SafeTune_warning(f"Missing required columns for {final_data_task}: {missing_cols}")
            print(f"  Existing heuristic mapping may have failed. Available columns: {dataset.column_names}")
        else:
            SafeTune_success(f"Schema validation passed for {final_data_task}.")
    ...
    elif config.task_type and config.task_type.lower() == "safety":
        logger.info("Task type 'safety': using safety suite metrics (harmfulness, over_refusal, jailbreak_success, capability_regression)")
        custom_metrics = get_metrics_from_names(EvalRegistry.get_safety_suite_metrics(), k_list=config.k_list)
    ...
    SafeTune_info("Starting evaluation...")
    print(dataset)
    ...
    SafeTune_success(f"Results saved to {results_path}")
