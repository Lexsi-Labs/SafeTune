...
from safetune.data.schemas import TASK_SCHEMAS, TaskType as SchemaTaskType
...
class BaseEvaluator:
    ...
    def __init__(
        ...
        if metrics is None:
            ...
            elif self.task_type in ["safety", "safety_eval"]:
                logger.info("Task type 'safety' detected. Using Harmfulness, OverRefusal, and CapabilityRegression metrics.")
                from .metrics.safety import HarmfulnessMetric, OverRefusalMetric, CapabilityRegressionMetric
                self.metrics = [HarmfulnessMetric(), OverRefusalMetric(), CapabilityRegressionMetric()]
            else:
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(dataloader, desc="Evaluating"):
                ...
                if isinstance(inputs[0], str):
                    ...
                    # Per-sample mean token loss (HF perplexity formula, WITHOUT the
                    # final exp — PerplexityMetric.compute expects raw losses and
                    # applies exp(mean(loss)) itself; exponentiating here too would
                    # double-exponentiate).
                    ...
                    all_losses.extend(loss_batch.cpu().tolist())
                    ...
                    # Raw per-sample mean loss — PerplexityMetric applies the exp.
                    loss_batch = (
                        (loss_fct(shift_logits.transpose(1, 2), shift_labels) * shift_attention_mask_batch).sum(1)
                        / shift_attention_mask_batch.sum(1)
                    )
        ...
        if evaluated == 0:
            logger.error(
                "No samples were evaluated — no batch yielded a recognizable "
                "prompt column. Pass column_mapping= to name your columns."
            )

        # Never cache empty or error-bearing results: a cached {metric_error: 0.0}
        # would be returned as a hit on every later run, hiding the failure.
        metric_failed = any(k.endswith("_error") for k in results)
        if metric_failed:
            logger.error(
                "One or more metrics failed (%s) — results NOT cached.",
                [k for k in results if k.endswith("_error")],
            )

        if self.cache and evaluated > 0 and not metric_failed:
            cache_args = {"task": current_task, "max_samples": max_samples}
            if column_mapping:
                cache_args["column_mapping"] = str(sorted(column_mapping.items()))
