"""
Core evaluation system for SafeTune.
            ...
            for i in iterable:
                ...
                if task.category in [TaskCategory.TEXT_GENERATION, TaskCategory.SUMMARIZATION, 
                    ...
                    outputs = model.generate(
                        **inputs,
                        # max_new_tokens (generation budget), NOT max_length (a
                        # TOTAL cap — which yields 0 new tokens once the prompt
                        # fills it).
                    ...
                    # Slice off the prompt so the prediction is only the
                    # generated continuation (decoding `outputs` whole echoes
                    # the prompt into every metric).
