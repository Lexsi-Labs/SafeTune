...
* **Safety suite:** Unified safety evaluation via `harmfulness`, `over_refusal`, and `capability_regression`. Use preset `EvalRegistry.get_safety_suite_metrics()` to run all three and log consistently (e.g. one JSON or table per run).
...
```{python}
from safetune.eval import BaseEvaluator
from safetune.eval.metrics import RougeMetric, BleuMetric
...
```{python}
from safetune.eval import RLEvaluator
...
└── metrics/            # Strategy Pattern Implementations
    ...
    └── safety.py       # Harmfulness, Over-refusal, Capability regression
```

### Safety suite

To run the unified safety evaluation (harmfulness, over-refusal, capability regression) in one go:

```{python}
from safetune.eval import registry
from safetune.eval.metrics import compute_safety_suite

# Get metric names for the preset
safety_metrics = registry.EvalRegistry.get_safety_suite_metrics()
# ["harmfulness", "over_refusal", "capability_regression"]

# Or compute all three at once (e.g. after collecting model predictions on benign + capability sets)
results = compute_safety_suite(
    predictions=model_outputs,
    references=references,
    harmfulness_scores=optional_classifier_scores,
    baseline_scores=optional_baseline_scores_for_regression,
)
# results: harmfulness_rate, harmfulness_mean, over_refusal_rate, capability_regression_delta, ...
```
...
```{python}
from safetune.eval import Metric
import numpy as np
