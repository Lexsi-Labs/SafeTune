...
class RLEvaluator(BaseEvaluator):
    ...
    def evaluate_rl(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="RL Eval"):
                if isinstance(batch, dict):
                    if first_batch:
                        logger.debug("Batch keys: %s; prompt candidates: %s; target candidates: %s",
                                     list(batch.keys()), prompt_keys, target_keys)
                    ...
                    if first_batch:
                        if not prompts:
                            logger.warning(
                                "No prompts extracted from first batch (keys: %s) — "
                                "batches without prompts are skipped. Check your "
                                "dataset columns / column_mapping.",
                                list(batch.keys()),
                            )
                        logger.debug("Extracted %d prompts / %d targets", len(prompts), len(targets))
                        first_batch = False
                ...
                if has_perplexity:
                    # Save current padding (outside the try so the finally-restore
                    # below can never hit an unassigned name)
                    ...
                    try:
                        ...
                        # Restore padding even on failure — a stuck 'right' padding
                        # silently corrupts all subsequent batched generation.
                    ...
                    try:
                        ...
                        # Used when no external reward model is provided, common for DPO eval
                            ...
                            try:
                                ...
                                # Restore padding even on failure (see perplexity block)
        ...
        logger.debug(
            "Collected data: %d losses, %d predictions, %d references, "
            "%d kl_divs, %d entropies, %d reward_pairs",
            len(all_losses), len(all_predictions), len(all_references),
            len(kl_divs), len(entropies), len(reward_pairs),
        )
